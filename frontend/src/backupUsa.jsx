...
const UsaChoroplethMaps = ({ parsedData, colorscale = 'Blues', files=[] }) => {
            ...
            <div style={{display: 'flex', direction: 'column'}}>
                <div style={{width: '50%'}}>
                    ...
                    {Object.keys(mappedFiles).length >0 && fileNamesInOrder.map((fileName, i) => (
                        <img key={i} src={URL.createObjectURL(mappedFiles[fileName])} alt={fileName} />
                    ))}
                </div>
                <div style={{width: '50%'}}>
                    {maps.map(map => (
                        <div key={map.id} id={map.id} className="map-container mb-10 h-96"></div>
                    ))}
                </div>
            </div>
